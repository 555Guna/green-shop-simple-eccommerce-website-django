...
class ProductView(View):
	def get(self, request):
		...
		return render(request, 'app/home.html', {'fashion':fashion, 'electronics':electronics, 'mobiles':mobiles, 'totalitem':totalitem})
...
def search(request):
	if request.method=="POST":
		...
		product = Product.objects.filter(brand__contains=searched) | Product.objects.filter(title__contains=searched)
		return render(request,'app/search.html',{'searched':searched,'product':product})
	else:
...
def fashion(request, data=None):
	...
	if data==None :
			...
			fashion = Product.objects.filter(category='F').filter(discounted_price__lt=5000)
	elif data == 'above':
			fashion = Product.objects.filter(category='F').filter(discounted_price__gt=10000)
	return render(request, 'app/fashion.html', {'fashion':fashion, 'totalitem':totalitem})
...
def appliances(request, data=None):
	...
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user))
	if data==None :
			appliances = Product.objects.filter(category='A')
	elif data == 'below':
			appliances = Product.objects.filter(category='A').filter(discounted_price__lt=50000)
	elif data == 'above':
			appliances = Product.objects.filter(category='A').filter(discounted_price__gt=50000)
...
def electronics(request, data=None):
	...
	if data==None :
			electronics = Product.objects.filter(category='E')
	elif data == 'LG' or data == 'SONY':
			electronics = Product.objects.filter(category='E').filter(brand=data)
	elif data == 'below':
			electronics = Product.objects.filter(category='E').filter(discounted_price__lt=50000)
	elif data == 'above':
			electronics = Product.objects.filter(category='E').filter(discounted_price__gt=50000)
...
def furniture(request, data=None):
	...
	if request.user.is_authenticated:
		totalitem = len(Cart.objects.filter(user=request.user))
	if data==None :
			furniture = Product.objects.filter(category='FT')
	elif data == 'below':
			furniture = Product.objects.filter(category='FT').filter(discounted_price__lt=50000)
	elif data == 'above':
			furniture = Product.objects.filter(category='FT').filter(discounted_price__gt=50000)
